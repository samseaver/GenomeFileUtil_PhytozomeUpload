...
#        cls.wsClient.create_workspace({'workspace': cls.wsName})
    ...
    def test_phytozome_to_genome(self):
        ...
        #Read Species Names
        SN_File = os.path.join(self.dtn_root,'..','Phytozome_Names.txt')
        Species_Names_Dict={}
        ...
        GM_File = os.path.join(self.dtn_root,'..','Accepted_Phytozome_Versions_GeneModels.txt')
        Species_Dict=dict()
        with open(GM_File) as f:
            for line in f:
                line=line.strip('\r\n')
                (Phytozome_Release,Species,Phytozome_Identifier,GeneModel_Version)=line.split('\t')

                if(Species not in Species_Dict):
                    Species_Dict[Species]=dict()
                Species_Dict[Species][GeneModel_Version]={'release':Phytozome_Release,'identifier':Phytozome_Identifier}

        Species_Versions_to_Skip=dict()
        with open('/kb/module/genomes/Phytozome_Summary.txt') as f:
            for line in f:
                line=line.strip()
                array = line.split('\t')
                array = array[0].split(':')
                Species_Version = array[1].strip()
                Species_Versions_to_Skip[Species_Version]=1

        summary_file=open('/kb/module/genomes/Phytozome_Summary.txt','w')
        for species in sorted(Species_Dict):

            if('Ahypogaea' not in species):
                continue
            ...
            for version in sorted(Species_Dict[species]):
                ...
                if(has_assembly == 0):
                    for versions in os.listdir(path):
                        if(versions == version):
                            for files in os.listdir(os.path.join(path,versions)):
                                if(files=="assembly"):
                ...
                summary_file.write('Functions: '+Genome_Name+'\t'+functions_file+'\t'+str(len(list(Functions)))+'\n')
                ...
                if(len(list(Functions))>0):
                    ...
                    #If no features were annotated, and mrnas were annotated, use parent_gene to do transfer annotation
                    ...
                    summary_file.write('Functions: '+Genome_Name+'\t'+functions_file+'\t'+str(Found_Count)+'\n')
                ...
                summary_file.write('Ontology: '+Genome_Name+'\t'+ontology_file+'\t'+str(len(Feature_Ontology.keys()))+'\n')
                summary_file.write('Ontology: '+Genome_Name+'\t'+ontology_file+'\t'+str(len(mRNA_Ontology.keys()))+'\n')
                ...
                summary_file.write('Ontology: '+Genome_Name+'\t'+ontology_file+'\t'+str(Found_Count)+'\n')
                ...
                summary_file.write('Synonyms: '+Genome_Name+'\t'+names_file+'\t'+str(len(list(Synonyms)))+'\n')
                ...
                summary_file.write('Synonyms: '+Genome_Name+'\t'+names_file+'\t'+str(Found_Count)+'\n')
                ...
                save_result = self.getImpl().save_one_genome(self.getContext(), {'workspace' : self.wsName,
                                                                                 'name' : Genome_Name,
                                                                                 'data' : Genome_Object,
                                                                                 'upgrade' : 1})
