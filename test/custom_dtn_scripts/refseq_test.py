...
#        cls.wsClient.create_workspace({'workspace': cls.wsName})
        ...
        cls.dtn_root = "/kb/module/data/RefSeq/" #Phytozome/V11/"
        print cls.dtn_root
    ...
    def test_phytozome_to_genome(self):
        ...
        Species_List = os.listdir(self.dtn_root)

        print "Testing with ",Species_List

        for Species in Species_List:
            if(Species != "Athaliana"):
                continue
            Species_Dir = os.path.join(self.dtn_root,Species)
            ...
            print "Annotation: ",Annotation_List
            Annotation_File = ""
            for File in Annotation_List:
                if('_genomic.gff.gz' in File):
                    Annotation_File = File
                    break

            Assembly_List = os.listdir(Assembly_Dir)
            print "Assembly: ",Assembly_List
            Assembly_File = ""
            for File in Assembly_List:
                if('_genomic.fna.gz' in File):
                    print "Assembly File:",File
                    Assembly_File = File
                    break

            Genome_Name = Species+"_NCBI_"+GeneModel_Version
